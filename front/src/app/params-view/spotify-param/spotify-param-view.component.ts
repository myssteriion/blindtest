...
export class SpotifyParamViewComponent implements OnInit {
    ...
    /**
     * The SpotifyParams before update.
     */
    private spotifyParamsBeforeUpdate: SpotifyParam;
    ...
    private findSpotifyParam(): void {
        ...
        this._spotifyParamResource.find().subscribe(
            response => {
                ...
                this.spotifyParamsBeforeUpdate = JSON.parse( JSON.stringify(response) );
                this.isLoaded = true;
            },
            error => {
    ...
    /**
     * Test if the spotify param was modified.
     */
    public spotifyParamWasModified(): boolean {
        return this.spotifyParamLoginWasModified() || this.spotifyParamPlaylistWasModified();
    }
    
    private spotifyParamLoginWasModified(): boolean {
        return this.spotifyParams.clientId !== this.spotifyParamsBeforeUpdate.clientId ||
            this.spotifyParams.clientSecret !== this.spotifyParamsBeforeUpdate.clientSecret;
    }
    
    private spotifyParamPlaylistWasModified(): boolean {
        
        let areSame = true;
        
        let keys: string[] = Object.keys(this.spotifyParamsBeforeUpdate.playlistIds);
        for (const key of keys) {
            let isSame = this.spotifyParamsBeforeUpdate.playlistIds[key] === this.spotifyParams.playlistIds[key];
            areSame = areSame && isSame;
        }
